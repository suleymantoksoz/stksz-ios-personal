...
function shouldExcludeFile(fileName) { return ['package-lock.json', 'yarn.lock', 'pnpm-lock.yaml'].some(ex => fileName === ex); }
function shouldScanFile(fileName) { if (['package-lock.json', 'yarn.lock', 'pnpm-lock.yaml'].some(ex => fileName === ex)) return false; return ['.js', '.ts', '.tsx', '.jsx', '.html', '.json', '.gradle', '.xml', '.xml', '.plist', '.podspec', '.java', '.kt', '.swift', '.m', '.mm', '.h'].some(ext => fileName.endsWith(ext)); }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        if (!['node_modules', 'Pods', 'build', 'DerivedData', '.git', '.gradle', '.idea', '.vscode', 'dist', 'coverage', '.nyc_output', 'temp', 'tmp'].some(ex => entry.name === ex)) walkDir(fullPath, fileList);
      } else if (entry.isFile()) {
        if (shouldScanFile(entry.name)) fileList.push(fullPath);
      }
    } catch (e) {}
    return fileList;
  }
