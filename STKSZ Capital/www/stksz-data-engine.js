...
(function initStkszDataEngine(global) {
  ...
  var FEATURE_FLAG_DEFAULTS = Object.freeze({
    ...
    /* M61: AdMob & reklam */
    feature_ads: true,
    feature_ads_free_user: true,
    feature_ads_pro: false,
    feature_ads_elite: false,
    /* M61 finalize (Item 20): birim bazlı + üst katman ad-imtiyazı */
    feature_ads_banner: true,
    feature_ads_interstitial: true,
    feature_ads_native: true,
    feature_ads_mediation: false,
    feature_ads_pro_free: false,
    feature_ads_elite_free: false
  });
  ...
  /* ===================================================================
     M61: ADMOB & AD REVENUE ALTYAPISI
     - Merkezi AdMob yapılandırma + ortam yöneticisi (dev/staging -> TEST
       reklam kimlikleri; production -> backend/kullanıcı yapılandırması).
     - Hesap denetim & kontrol listesi motoru (uygulama bağlama, yayıncı
       kimliği, ödeme profili, app-ads.txt, mediation).
     - app-ads.txt üretimi + doğrulama hattı (HTTP 200, kök yolu, SSL,
       biçim, yönlendirme, robots).
     - M60 rol + feature flag kapı entegrasyonu (feature_ads*).
     - Mediation/Bidding adaptör yuvaları + boş analitik rapor stubları.
     NOT: Yayıncı/test kimliği üretilmez; yalnız Google'ın resmî TEST
     reklam kimlikleri (genel dokümantasyon) development/staging için
     kullanılır. Production kimlikleri yapılandırmadan gelir.
     =================================================================== */
  const ADMOB_ENVS = Object.freeze(['development', 'staging', 'production']);
  /* Google official TEST ad unit IDs (public docs) */
  const ADMOB_TEST_IDS = Object.freeze({
  ...
  const ADMOB_UNIT_KEYS = Object.freeze(['ADMOB_APP_ID', 'ADMOB_BANNER_ID', 'ADMOB_INTERSTITIAL_ID', 'ADMOB_NATIVE_ID', 'ADMOB_REWARDED_ID']);
  function validAdmobEnv(env) { const e = String(env || '').toLowerCase(); return ADMOB_ENVS.includes(e) ? e : 'development'; }
  /* userConfig: { ADMOB_APP_ID, ADMOB_BANNER_ID, ... } — production'da gerçek
     kimlikler YALNIZ bu yapılandırmadan gelir; yoksa TEST kimliğe düşmez,
     eksik olduğunu raporlar. */
  function resolveAdIds(env, userConfig) {
    env = validAdmobEnv(env);
    const cfg = userConfig && typeof userConfig === 'object' ? userConfig : {};
    if (env === 'production') {
      const needs = ADMOB_UNIT_KEYS.filter(k => !cfg[k]);
      return { env, testMode: false, ids: Object.assign({}, cfg), missing: needs };
    }
    const merged = {};
    ADMOB_UNIT_KEYS.forEach(k => { merged[k] = cfg[k] || ADMOB_TEST_IDS[k]; });
    return { env, testMode: true, ids: merged, missing: [] };
  }
  function admobConfigSnapshot(env, userConfig) {
    const r = resolveAdIds(env, userConfig);
    return {
      env: r.env,
      testMode: r.testMode,
      appId: r.ids.ADMOB_APP_ID || null,
      adUnitTypes: ['banner', 'interstitial', 'native', 'rewarded'],
      missing: r.missing,
      note: r.testMode ? 'TEST reklam kimlikleri — canlı gösterim YOK.' : (r.missing.length ? 'Eksik production kimlikleri: ' + r.missing.join(', ') : 'Production kimlikleri yapılandırıldı.')
    };
  }
  /* ---- Hesap denetim & kontrol listesi ---- */
  function admobAudit(config) {
    config = config || {};
    const env = validAdmobEnv(config.env);
    const pub = String(config.publisherId || '').trim();
    const hasStore = Boolean(config.bundleId && config.bundleId.trim());
    const items = [
      { id: 'binding', label: 'Uygulama mağaza bağlama (Google Play / App Store Bundle ID)', ok: hasStore, detail: hasStore ? config.bundleId : 'Bundle ID yapılandırılmadı' },
      { id: 'publisher', label: 'Yayıncı kimliği kaydı (pub-XXXXXXXXXXXX)', ok: /^pub-[0-9]{16}$/.test(pub), detail: pub || 'pub- kimliği girilmedi' },
      { id: 'payment', label: 'Ödeme profili tamamlandı (AdSense/AdMob', ok: Boolean(config.paymentProfile), detail: config.paymentProfile ? 'Tamamlandı' : 'Ödeme profili bilinmiyor' },
      { id: 'appads', label: 'app-ads.txt varlığı & web sunucusu doğrulama', ok: Boolean(config.appAdsTxtVerified), detail: config.appAdsTxtVerified ? 'Doğrulandı' : 'Doğrulanmadı / kök yolu gerekli' },
      { id: 'mediation', label: 'Mediation / Bidding hazırlığı', ok: Boolean(config.mediationReady), detail: 'Mediation adaptör yuvaları hazır; etkin ağ yok' }
    ];
    const score = Math.round(items.filter(i => i.ok).length / items.length * 100);
    return { score, items, note: env === 'development' || env === 'staging' ? 'Test ortamı — canlı reklam yok.' : 'Production doğrulama yönergesi.' };
  }
  /* ---- app-ads.txt üretimi & doğrulama hattı ---- */
  function appAdsTxtPublisherLine(publisherId) {
    const pub = String(publisherId || '').trim();
    if (!/^pub-[0-9]{16}$/.test(pub)) return null;
    return 'google.com, ' + pub + ', DIRECT, f08c47fec0942fa0';
  }
  function buildAppAdsTxt(publisherId, resellers) {
    const main = appAdsTxtPublisherLine(publisherId);
    if (!main) return { ok: false, lines: [], error: 'Geçersiz google.com:pub yayıncı kimliği.' };
    const lines = [main, '#' + new Date().toISOString().slice(0, 10)];
    (Array.isArray(resellers) ? resellers : []).forEach(r => {
      if (r && r.line && r.authorized) lines.push(r.line);
    });
    return { ok: true, lines, urlTemplate: 'https://[DOMAIN]/app-ads.txt' };
  }
  function checkRobotsAllowsRobotsTxt() { return true; } /* yer tutucu: kök robots.txt engellemiyor */
  async function verifyAppAdsTxt(baseUrl, publisherId) {
    if (typeof fetch === 'undefined') return { ok: false, error: 'fetch yok — ağ doğrulaması kapalı.' };
    try {
      const url = String(baseUrl || '').replace(/\/+$/, '') + '/app-ads.txt';
      const res = await fetch(url, { redirect: 'manual' });
      const checks = {
        http200: res.status === 200,
        rootPath: url.indexOf('/app-ads.txt') === url.length - 11,
        ssl: /^https:\/\//i.test(url),
        format: false,
        noRedirect: (res.type === 'basic') || !res.redirected,
        robots: checkRobotsAllowsRobotsTxt()
      };
      if (res.status === 200) {
        const text = await res.text();
        const expected = appAdsTxtPublisherLine(publisherId);
        checks.format = Boolean(expected) && String(text || '').indexOf(expected) !== -1;
      }
      return { ok: checks.http200 && checks.rootPath && checks.ssl && checks.format && checks.noRedirect, url, checks };
    } catch (e) { return { ok: false, error: 'Doğrulama başarısız: ' + (e && e.message) }; }
  }
  /* ---- MODULE 64 & 64-A: Telegram ödeme sağlayıcıları + uyumluluk motoru (fonksiyon implementasyonu) ---- */
/* Provider durum haritası (backend Zustand + opsiyonel localStorage iyilemesi) */
  let telegramProviderStates = {};
  function telegramPaymentProviderStatus(providerId) {
    const provider = TELEGRAM_PAYMENT_PROVIDERS[providerId];
    if (!provider) return { ok: false, error: 'Bilinmeyen sağlayıcı' };
    const state = telegramProviderStates[providerId] || { status: 'unknown', lastCheck: null, testResult: null };
    return { ok: true, provider: { id: provider.id, name: provider.name, kind: provider.kind }, state };
  }
  function telegramSelectProvider({ country, currency, paymentType }) {
    const candidates = [];
    Object.keys(TELEGRAM_PAYMENT_PROVIDERS).forEach(k => {
      const p = TELEGRAM_PAYMENT_PROVIDERS[k];
      if (p.status !== 'active') return;
      candidates.push({ provider: p, state: telegramProviderStates[k] || { status: 'unknown' } });
    });
    if (candidates.length === 0) return null;
    return candidates[0];
  }
  /* Backend Vault'ına invoice başlatma isteği → frontend token yok */
  async function telegramInitiateInvoice({ providerId, priceTRX, payload }) {
    const providerStatus = telegramPaymentProviderStatus(providerId);
    if (!providerStatus.ok) return { ok: false, error: 'Sağlayıcı aktif değil' };
    return {
      ok: true,
      provider: providerStatus.provider,
      redirectUrl: '/payment/initiate?' + new URLSearchParams({ provider: providerId, price: priceTRX, payload: JSON.stringify(payload) }).toString(),
      note: 'Backend Vault üzerinden ödeme akışı başlatıldı; frontend token yok.'
    };
  }
  /* Backend successful_payment webhook doğrulaması */
  async function telegramVerifyPayment({ providerId, telegramPaymentId, serverSignature }) {
    if (window && window.fetch) {
      const r = await window.fetch('/api/payment/verify', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ providerId, telegramPaymentId, serverSignature })
      });
      const payload = await r.json().catch(() => ({}));
      return { ok: r.ok, status: r.status, payload };
    }
    return { ok: false, error: 'fetch yok' };
  }
  /* Successful payment webhook — PAYMENT_SUCCESS/SUBSCRIPTION_CHECK pipeline trigger */
  function telegramPaymentWebhook({ providerId, transactionId, status, duration, userId }) {
    if (window && window.STKSZAIEngine && window.STKSZAIEngine.entitlements && window.STKSZAIEngine.entitlements.grant) {
      if (status === 'success') {
        window.STKSZAIEngine.entitlements.grant('STKSZ_PRO', 'system:payment');
        window.STKSZAIEngine.entitlements.grant('STKSZ_ELITE', 'system:payment_elite');
      }
      if (window.STKSZAIEngine.workflow && window.STKSZAIEngine.workflow.recordOrder) {
        window.STKSZAIEngine.workflow.recordOrder({ id: transactionId, type: 'subscription', status, duration, userId });
      }
    }
    if (window && window.fetch) {
      window.fetch('/api/payment/success', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ providerId, transactionId, status, duration, userId })
      });
    }
  }
  /* ---- M60 rol + feature flag kapı entegrasyonu ---- */
  /* Item 22: strict tier davranışı — GUEST/FREE reklama tabi; PRO/ELITE varsayılan
     ad-free (yayın ve ağ isteği YAPILMAZ) yalnız feature_ads_*_free true ise gösterilir;
     ADMIN her zaman ad-free (görünüm modu testinden ayrıdır; DB rol değişmez). */
  function adUnitAllowed(unit, role, flags) {
    flags = flags || {};
    const r = String(role || 'free').toLowerCase();
    const unitFlag = 'feature_ads_' + String(unit || '').toLowerCase();
    if (unit === 'banner' && !flags.feature_ads_banner) return false;
    if (unit === 'interstitial' && !flags.feature_ads_interstitial) return false;
    if (unit === 'native' && !flags.feature_ads_native) return false;
    if (r === 'pro' && !flags.feature_ads_pro_free) return false;
    if (r === 'elite' && !flags.feature_ads_elite_free) return false;
    return flags.feature_ads !== false && (flags[unitFlag] !== false);
  }
  function adsEnabledFor(role, flags) {
    flags = flags || {};
    if (flags.feature_ads === false) return { show: false, reason: 'feature_ads kapalı (global)' };
    const r = String(role || 'free').toLowerCase();
    if (r === 'admin') return { show: false, reason: 'admin reklamsız' };
    if (r === 'guest' || r === 'free') {
      return flags.feature_ads_free_user === true
        ? { show: true, reason: 'free/guest: feature_ads_free_user açık' }
        : { show: false, reason: 'feature_ads_free_user kapalı' };
    }
    if (r === 'pro') return flags.feature_ads_pro_free === true
      ? { show: true, reason: 'PRO ad-free; admin pro_free üst katmanı açtı' }
      : { show: false, reason: 'PRO ad-free (feature_ads_pro_free kapalı)' };
    if (r === 'elite') return flags.feature_ads_elite_free === true
      ? { show: true, reason: 'ELITE ad-free; admin elite_free üst katmanı açtı' }
      : { show: false, reason: 'ELITE ad-free (feature_ads_elite_free kapalı)' };
    return { show: false, reason: 'bilinmeyen rol' };
  }
  /* Birim bazlı net karar (banner/interstitial/native) — hiçbir ikamesi/gerçek
     değeri üretmez; yalnız 0/1 kuralları. Feature tabanlı tutarlılık için global
     kapı ile birleşik sonuç döner. */
  function adsUnitPlan(role, flags) {
    flags = flags || {};
    const base = adsEnabledFor(role, flags);
    const r = String(role || 'free').toLowerCase();
    const plan = {
      show: base.show,
      reason: base.reason,
      entitlementOk: !(r === 'pro' || r === 'elite') || (flags.feature_ads_pro_free === true || flags.feature_ads_elite_free === true),
      units: {
        banner: adUnitAllowed('banner', role, flags),
        interstitial: adUnitAllowed('interstitial', role, flags),
        native: adUnitAllowed('native', role, flags)
      },
      mediation: flags.feature_ads_mediation === true
    };
    if (!base.show) { plan.units.banner = false; plan.units.interstitial = false; plan.units.native = false; }
    return plan;
  }
  /* ---- Item 28: Policy Center diagnostics (AdMob reddi/politika uyarısı) ----
     Yalnız OLAYI günlüğe kaydeder; rastgele UI/kod değişikliği YAPMAZ. */
  const AD_DIAG_KEY = 'stkszAdPolicyDiag';
  function logAdPolicyEvent(event, detail) {
    event = String(event || 'unknown');
    const entry = { ts: new Date().toISOString(), event, detail: String(detail || ''), env: (typeof location !== 'undefined' ? location.hostname : '') };
    let arr = [];
    if (typeof localStorage !== 'undefined') { try { const raw = localStorage.getItem(AD_DIAG_KEY); if (raw) { const p = JSON.parse(raw); if (Array.isArray(p)) arr = p; } } catch (e) {} }
    arr.push(entry);
    if (arr.length > 200) arr = arr.slice(-200);
    if (typeof localStorage !== 'undefined') { try { localStorage.setItem(AD_DIAG_KEY, JSON.stringify(arr)); } catch (e) {} }
    return { ok: true, count: arr.length };
  }
  function adPolicyStatus() {
    let arr = [];
    if (typeof localStorage !== 'undefined') { try { const raw = localStorage.getItem(AD_DIAG_KEY); if (raw) { const p = JSON.parse(raw); if (Array.isArray(p)) arr = p; } } catch (e) {} }
    const events = arr.map(a => a.event);
    const counts = {};
    events.forEach(e => { counts[e] = (counts[e] || 0) + 1; });
    return { count: arr.length, events: counts, recent: arr.slice(-10).reverse() };
  }
  function clearAdPolicyDiag() { if (typeof localStorage !== 'undefined') { try { localStorage.removeItem(AD_DIAG_KEY); } catch (e) {} } return { ok: true }; }
  /* ---- Item 26: Store parametreleri ile yapı taşı uyumu ---- */
  function storeIntegrityCheck(cfg) {
    cfg = cfg || {};
    const bundle = String(cfg.bundleId || '').trim();
    const name = String(cfg.appName || '').trim();
    const domain = String(cfg.developerDomain || '').trim();
    const privacy = String(cfg.privacyUrl || '').trim();
    const support = String(cfg.supportUrl || '').trim();
    const items = [
      { id: 'bundle', label: 'Uygulama Adı & Bundle ID eşleşmesi', ok: Boolean(bundle && name), detail: (bundle || 'Bundle ID yok') + ' · ' + (name || 'Uygulama adı yok') },
      { id: 'domain', label: 'Geliştirici Web Alanı (store listing)', ok: /^https:\/\//.test(domain), detail: domain || 'Örn. https://stksz.com girilmeli' },
      { id: 'privacy', label: 'Gizlilik Politikası URL', ok: /^https:\/\//.test(privacy), detail: privacy || 'https politikası yok' },
      { id: 'support', label: 'Destek URL', ok: /^https:\/\//.test(support), detail: support || 'Destek URL yok' }
    ];
    const score = Math.round(items.filter(i => i.ok).length / items.length * 100);
    return { score, items, note: 'Store parametreleri AdMob bağlama ve politika uyumu için kontrol edildi.' };
  }
  /* ---- Item 27: Gizlilik & yasal sorumluluk kapsamı (AdMob SDK + AI/finans) ---- */
  function privacyCoverageCheck() {
    return {
      coverages: [
        { id: 'admob_sdk', label: 'AdMob SDK & işleme (reklam kimliği, çerez/cihaz tanımlayıcı)', ok: true, detail: 'Gizlilik Politikası AdMob SDK kullanımını kapsar.' },
        { id: 'consent', label: 'Avrupa BKA / ABD eyalet onayı akışı', ok: false, detail: 'UUCA/CCPA onay akışı henüz bağlanmadı (AdMob uyumluluk)' },
        { id: 'ai_scenario', label: 'Tüm AI/finans sonuçlarında "Yatırım tavsiyesi değildir" zorunlu', ok: true, detail: 'AI raporları ve senaryo çıktıları yasal feragatname taşır.' },
        { id: 'data_min', label: 'Kişisel finans verisi günlüğe/AI anahtarına asla yazılmaz', ok: true, detail: 'API anahtarları yalnız yapılandırmada; senkron dışa saklanır.' }
      ],
      mandatoryDisclaimer: 'Bu içerik bilgilendirme ve senaryo çalışması amaçlıdır; yatırım tavsiyesi değildir. Yapay zeka analizleri haber ve kamuoyu bilgilerine dayanır; nihai kararlar siz sorumlusunuz.'
    };
  }
  /* ---- Mediation / Bidding adaptör yuvaları (boş, güvenli) ---- */
  const MEDIATION_ADAPTERS = Object.freeze({
    Meta:   { id: 'meta',     configured: false, authorizedReseller: false },
    AppLovin:{ id: 'applovin', configured: false, authorizedReseller: false },
    Unity:  { id: 'unity',    configured: false, authorizedReseller: false }
  });
  function mediationAdapter(network) {
    const n = String(network || '');
    const found = Object.keys(MEDIATION_ADAPTERS).find(k => MEDIATION_ADAPTERS[k].id === n.toLowerCase());
    return found ? MEDIATION_ADAPTERS[found] : null;
  }
  /* ---- Admin analitik rapor stubları (SAHTE rakam ÜRETMEZ) ---- */
  function admobReportingStubs() {
    return {
      metrics: { estimatedRevenue: null, impressions: null, clicks: null, ctr: null, ecpm: null, fillRate: null },
      countryBreakdown: [],
      note: 'Rakamlar yalnız AdMob/reporting entegrasyonu etkinleşince dolar; burada veri üretilmez.'
    };
  }
  ...
  const api = {
    ...
    /* M61: AdMob & ad revenue altyapısı */
    ADMOB_ENVS, ADMOB_TEST_IDS, ADMOB_UNIT_KEYS, resolveAdIds, admobConfigSnapshot,
    admobAudit, appAdsTxtPublisherLine, buildAppAdsTxt, verifyAppAdsTxt,
    adsEnabledFor, MEDIATION_ADAPTERS, mediationAdapter, admobReportingStubs,
    adUnitAllowed, adsUnitPlan, logAdPolicyEvent, adPolicyStatus, clearAdPolicyDiag,
    storeIntegrityCheck, privacyCoverageCheck,
